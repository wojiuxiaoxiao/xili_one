...
class ModuleObject extends AppObject{
	...
	function topicHome(){
		//热门话题
		$topicListTmp = $this->Db->fetch_all_assoc("SELECT topic_id,topic_name FROM `topic_list` where isHot>0 order by isHot desc limit 4");
		//板块列表
		$forumHot = array();
	...
	function hotTopic(){
		...
		if($start>0){
			$topicListTmp = $this->Db->fetch_all_assoc("SELECT * FROM `topic_list` where isHot>0 and isHot<".$start." order by isHot desc limit 0,$amount");
		}
		else{
