...
class ModuleObject extends AppObject{
	...
	function getHotNews(){
		...
		foreach($hotNews as $k=>$news){
			...
			$hotNews[$k]['headpic'] = str_replace("http://www.yeeyi.com","https://m.yeeyi.com",$hotNews[$k]['headpic']);
		}
	...
	function getNewsList(){
		...
		$startTopicId =  intval($this->Post['startTopicId']);
		...
		if($cid>0){
			...
			$newsAry = $this->Db->fetch_all_assoc("select aid,title,pic,description,pubdate,froms,tourl,aid as orderid from news_article where ".$whereAid." cid in(".implode(',',$cidAry).") order by aid desc limit 0,".$amount);
		}
		else{
            if($startAid == 0){
                $newsAry = $this->Db->fetch_all_assoc("select aid,title,pic,description,pubdate,froms,tourl,essence as orderid from news_article where delstate=0 and essence>0 order by essence desc limit 0,".$amount);
            }
            else{
                $newsAry = $this->Db->fetch_all_assoc("select aid,title,pic,description,pubdate,froms,tourl,essence as orderid from news_article where delstate=0 and essence<$startAid and essence>0 order by essence desc limit 0,".$amount);
            }
		}

		$topicCount = ceil($amount/10);
        if($startTopicId>0){
            //$topicAry = $this->Db->fetch_all_assoc("select topic_id,topic_name,pic1,pic2,pic3 from topic_list where topic_id<$startTopicId order by topic_id desc limit 0,$topicCount");
        }
        else{
            //$topicAry = $this->Db->fetch_all_assoc("select topic_id,topic_name,pic1,pic2,pic3 from topic_list order by topic_id desc limit 0,$topicCount");
        }
		...
		foreach($newsAry as $news_key=>$news){
            if(strstr($news['tourl'],'http')){
				...
				$news['froms'] = strval($news['outlink']['author']);
            }
            unset($news['tourl']);
			$news['isNews'] = 1;
			if($news['pic']){
				...
				$news['pic'] = str_replace("http://www.yeeyi.com","https://m.yeeyi.com",$news['pic']);
			}
			$tmpAry[] = $news;
			if(++$step % 10 == 0){
				if(isset($topicAry[$i])) {
                    ...
                    if(!strstr($topicAry[$i]['pic1'],'http:')){
                        $topicAry[$i]['pic1'] = 'https://m.yeeyi.com/'.$topicAry[$i]['pic1'];
                    }
                    if(!strstr($topicAry[$i]['pic2'],'http:')){
                        $topicAry[$i]['pic2'] = 'https://m.yeeyi.com/'.$topicAry[$i]['pic2'];
                    }
                    if(!strstr($topicAry[$i]['pic3'],'http:')){
                        $topicAry[$i]['pic3'] = 'https://m.yeeyi.com/'.$topicAry[$i]['pic3'];
                    }
	...
	function getNewsContent(){
		...
		$newsAry = $this->Db->once_fetch_assoc("select news_article.aid,news_article.cid,news_article.title,froms,editor,pic,description,pubdate,comments,good,content,bancom,type_1,type_2,type_3,type_4 from news_article left join news_date on news_date.aid=news_article.aid where news_article.aid=".$aid);
		if($newsAry['aid']<1){
			$return['status'] = 5202;
			$this->outjson($return);
		}
		...
		//相关新闻
		$relevantNewsTmp = $this->Db->fetch_all_assoc("select aid,title,pic,description,pubdate,froms from news_article where cid=".$cid." and aid!=".$aid." order by aid desc limit 0,10");
		$relevantNews = array();
		foreach($relevantNewsTmp as $news){
			$news['isNews'] = 1;
			if($news['pic']){
				//$news['pic'] = $news['pic']."_nl.jpg";
			}
			$relevantNews[] = $news;
		}
