...
class ThreadPost
{
    ...
    function __construct(){
        ...
        $this->inPutCity = arrayValueToName($config['inPutCity']);
    ...
    /* 2017.05.12 edit by allen qu gb2312不支持繁体, 所以所有版块的所有字段都改为gbk */

    /* 2017.05.17 edit by allen qu 添加了团购打折的方法 325 */
    ...
    function post325(){
        ...
        //构建内容
        $tmp = '<table cellspacing="0" class="carcontent">';
        $tmp .= '<caption>团购打折</caption>';
        if(!empty($cityStr) || !empty($suburb) || !empty($postcode)){
            $tmp .='<tr><th class="tit">所在城市:</th><td class="con">'.$cityStr." ".$suburb." ".$postcode.'</td></tr>';
        }
        if(!empty($typeidStr)){
            $tmp .='<tr><th class="tit">类别:</th><td class="con">'.$typeidStr.'</td></tr>';
        }
        if(!empty($poster)){
            $tmp .='<tr><th class="tit">联系人:</th><td class="con">'.$poster.'</td></tr>';
        }
        if(!empty($tel)){
            $tmp .='<tr><th class="tit">电话:</th><td class="con">'.$tel.'</td></tr>';
        }
        if(!empty($weixin)){
            $tmp .='<tr><th class="tit">微信:</th><td class="con">'.$weixin.'</td></tr>';
        }
        if(!empty($qq)){
            $tmp .='<tr><th class="tit">QQ:</th><td class="con">'.$qq.'</td></tr>';
        }
        if(!empty($email)){
            $tmp .='<tr><th class="tit">邮箱:</th><td class="con">'.$email.'</td></tr>';
        }
        $tmp .="</table>";
        ...
        $suburb = changeCode($suburb,'utf-8','gbk');

        $message = changeCode($message,'utf-8','gbk');
        $poster = changeCode($poster,'utf-8','gbk');
        $tel = changeCode($tel,'utf-8','gbk');
        $weixin = changeCode($weixin,'utf-8','gbk');
        $qq = changeCode($qq,'utf-8','gbk');
        $email = changeCode($email,'utf-8','gbk');

        $sql = "insert into pre_forum_field_groupbuy(tid,city,suburb,postcode,typeid,tel,weixin,qq,email,message, poster) values('".$tid."', '".$city."', '".$suburb."', '".$postcode."', '".$typeid."', '".$tel."', '".$weixin."', '".$qq."', '".$email."', '".$message."', '".$poster."')";
    ...
    function update325(){
        ...
        //转换
        $cityInfo = $this->Db->once_fetch_assoc("select city from pre_forum_field_groupbuy where tid=".$tid);//城市不能修改
        $city = $cityInfo['city'];
        ...
        $tmp .= '<caption>团购打折</caption>';
        $tmp .='<tr><td class="tit">所在城市:</td><td class="con">'.$cityStr." ".$suburb." ".$postcode.'</td><td class="tit">类别:</td><td class="con2">'.$typeidStr.'</td></tr>';
        $tmp .='<tr><td class="tit">联系人:</td><td class="con">'.$poster.'</td></tr>';
        ...
        $sql = "update pre_forum_field_groupbuy set city='".$city."',suburb='".$suburb."',postcode='".$postcode."',typeid='".$typeid."',tel='".$tel."',weixin='".$weixin."',qq='".$qq."',poster='".$poster."', email='".$email."',message='".$message."'  where tid=".$tid;
        $result = $this->Db->query($sql);
        return $postThreadInfo;
    }

    /* 2017.05.17 edit by allen qu 添加了宠物交易的方法 604 */
    ...
    function post604(){
        ...
        //构建内容
        $tmp = '<table cellpadding="0" cellspacing="1" id="mytable">';
        $tmp .= '<caption>宠物交易</caption>';
        if(!empty($cityStr) || !empty($suburb) || !empty($postcode)){
            $tmp .='<tr><th class="tit">所在城市:</th><td class="con">'.$cityStr." ".$suburb." ".$postcode.'</td></tr>';
        }
        if(!empty($typeidStr)){
        ...
        if(!empty($icStr)){
            $tmp .='<tr><th class="tit">芯片:</th><td class="con">'.$icStr.'</td></tr>';
        }
        if(!empty($poster)){
            $tmp .='<tr><th class="tit">联系人:</th><td class="con">'.$poster.'</td></tr>';
        }
        ...
        $sql = "insert into pre_forum_field_pet(tid,city,suburb,postcode,typeid,tel,weixin,qq,email,message, poster, ic) values('".$tid."', '".$city."', '".$suburb."', '".$postcode."', '".$typeid."', '".$tel."', '".$weixin."', '".$qq."', '".$email."', '".$message."', '".$poster."', '".$ic."')";
        $result = $this->Db->query($sql);
        return $postThreadInfo;
    ...
    function update604(){
        ...
        $pid = intval($this->Post['pid']);
        //转换
        $cityInfo = $this->Db->once_fetch_assoc("select city from pre_forum_field_pet where tid=".$tid);//城市不能修改
        $city = $cityInfo['city'];
        $ic       = intval($this->Post['ic']);
        $nomalMessage = stripslashes($message);
        $dateline = time();
        ...
        $tmp .= '<caption>宠物交易</caption>';
        $tmp .='<tr><td class="tit">所在城市:</td><td class="con">'.$cityStr." ".$suburb." ".$postcode.'</td><td class="tit">类别:</td><td class="con2">'.$typeidStr.'</td></tr>';
        $tmp .='<tr><td class="tit">芯片:</td><td class="con">'.$icStr.'</td><td class="tit">联系人:</td><td class="con">'.$poster.'</td></tr>';
        $tmp .='<tr><td class="tit">电话:</td><td class="con2">'.$tel.'</td><td class="tit">微信:</td><td class="con">'.$weixin.'</td></tr>';
        $tmp .='<tr><td class="tit">QQ:</td><td class="con2">'.$qq.'</td><td class="tit">邮箱:</td><td class="con">'.$email.'</td></tr>';
        $tmp .="</table></div>";
        $tmp .="<p>".$nomalMessage."</p>";
        ...
        $sql = "update pre_forum_field_pet set city='".$city."',suburb='".$suburb."',postcode='".$postcode."',typeid='".$typeid."',ic='".$ic."',tel='".$tel."',weixin='".$weixin."',qq='".$qq."',poster='".$poster."', email='".$email."',message='".$message."'  where tid=".$tid;
        $result = $this->Db->query($sql);
        return $postThreadInfo;
    ...
    function post291(){
        ...
        if(!empty($cityStr) || !empty($suburb)){
            $tmp .='<tr><th class="tit">所在地区:</th><td class="con">'.$cityStr." ".$suburb.'</td></tr>';
        }
        if(!empty($carFromStr)){
            $tmp .='<tr><th class="tit">来源:</th><td class="con">'.$carFromStr.'</td></tr>';
        }
        if(!empty($carmake) || !empty($model)){
            $tmp .='<tr><th class="tit">品牌系列:</th><td class="con">'.$carmake." ".$model.'</td></tr>';
        }
        if(!empty($bodytypeStr)){
            $tmp .='<tr><th class="tit">车型:</th><td class="con">'.$bodytypeStr.'</td></tr>';
        }
        if(!empty($transmissionStr)){
            $tmp .='<tr><th class="tit">变速箱:</th><td class="con">'.$transmissionStr.'</td></tr>';
        }
        if(!empty($colourStr)){
            $tmp .='<tr><th class="tit">颜色:</th><td class="con">'.$colourStr.'</td></tr>';
        }
        if(!empty($drivesStr)){
            $tmp .='<tr><th class="tit">驱动:</th><td class="con">'.$drivesStr.'</td></tr>';
        }
        if(!empty($seatsStr)){
            $tmp .='<tr><th class="tit">座位数:</th><td class="con">'.$seatsStr.'座</td></tr>';
        }
        if(!empty($doorsStr)){
            $tmp .='<tr><th class="tit">门数:</th><td class="con2">'.$doorsStr.'</td></tr>';
        }
        if(!empty($fueltypeStr)){
            $tmp .='<tr><th class="tit">燃油类型:</th><td class="con">'. $fueltypeStr.'</td></tr>';
        }
        if(!empty($displace)){
            $tmp .='<tr><th class="tit">排量:</th><td class="con2">'.$displace.'升</td></tr>';
        }
        if(!empty($kilometres)){
            $tmp .='<tr><th class="tit">行驶里程:</th><td class="con">'.$kilometres.'公里</td></tr>';
        }
        if(!empty($havemaintenanceStr)){
            $tmp .='<tr><th class="tit">保养记录:</th><td class="con">'.$havemaintenanceStr.'</td></tr>';
        }
        if(!empty($price)){
            $tmp .='<tr><th class="tit">价格:</th><td class="con2">$'.$price.'</td></tr>';
        }
        if(!empty($marktime)){
            $tmp .='<tr><th class="tit" colspan="4">首次上牌日期:<span style="font-weight: normal;">'.$marktime.'</span></th></tr>';
        }
        $tmp .='<tr><th colspan="4" style="height: 28px;background: #E5EDF2;margin-left:20px;">&nbsp;</th></tr>';
        if(!empty($poster)){
            $tmp .='<tr><th class="tit">联系人:</th><td class="con">'.$poster.'</td></tr>';
        }
        if(!empty($tel)){
        ...
        if(!empty($email)){
            $tmp .='<tr><th class="tit">邮箱:</th><td colspan="3" class="con">'.$email.'</td></tr>';
        }
        $tmp .="</table>";
        $tmp .="<p>".$nomalMessage."</p>";
        $fid = 291;
        ...
        //编码转换
        $city = changeCode($city,'utf-8','gbk');
        $suburb = changeCode($suburb,'utf-8','gbk');
        $postcode = changeCode($postcode,'utf-8','gbk');
        $carfrom = changeCode($carfrom,'utf-8','gbk');
        $carmake = changeCode($carmake,'utf-8','gbk');
        $model = changeCode($model,'utf-8','gbk');
        $bodytype = changeCode($bodytype,'utf-8','gbk');
        $transmission = changeCode($transmission,'utf-8','gbk');
        $colour = changeCode($colour,'utf-8','gbk');
        $drives = changeCode($drives,'utf-8','gbk');
        $seats = changeCode($seats,'utf-8','gbk');
        $doors = changeCode($doors,'utf-8','gbk');
        $fueltype = changeCode($fueltype,'utf-8','gbk');
        $displace = changeCode($displace,'utf-8','gbk');
        $kilometres = changeCode($kilometres,'utf-8','gbk');
        $havemaintenance = changeCode($havemaintenance,'utf-8','gbk');
        $marktime = changeCode($marktime,'utf-8','gbk');
        $price = changeCode($price,'utf-8','gbk');
        $poster = changeCode($poster,'utf-8','gbk');
        $tel = changeCode($tel,'utf-8','gbk');
        $weixin = changeCode($weixin,'utf-8','gbk');
        $qq = changeCode($qq,'utf-8','gbk');
    ...
    function post715(){
        ...
        if(!empty($cityStr) || !empty($suburb) || !empty($postcode)){
            $tmp .='<tr><th class="tit">所在地区:</th><td class="con">'.$cityStr." ".$suburb.' '.$postcode.'</td></tr>';
        }
        if(!empty($carFromStr)){
            $tmp .='<tr><th class="tit">车辆来源:</th><td class="con">'.$carFromStr.'</td></tr>';
        }
        if(!empty($carmake) || !empty($model)){
            $tmp .='<tr><th class="tit">品牌系列:</th><td class="con">'.$carmake." ".$model.'</td></tr>';
        }
        if(!empty($bodytypeStr)){
            $tmp .='<tr><th class="tit">期望车型:</th><td class="con">'.$bodytypeStr.'</td></tr>';
        }
        if(!empty($carPriceStr)){
            $tmp .='<tr><th class="tit">期望价格:</th><td class="con">'.$carPriceStr.'</td></tr>';
        }
        if(!empty($carageStr)){
            $tmp .='<tr><th class="tit">期望车龄:</th><td class="con">'.$carageStr.'</td></tr>';
        }
        if(!empty($kilometresStr)){
            $tmp .='<tr><th class="tit">里程:</th><td class="con">'.$kilometresStr.'</td></tr>';
        }
        if(!empty($transmissionStr)){
            $tmp .='<tr><th class="tit">变速箱:</th><td class="con">'.$transmissionStr.'</td></tr>';
        }
        ...
        $model = changeCode($model,'utf-8','gbk');
        $carmake = changeCode($carmake,'utf-8','gbk');
        $carprice = changeCode($carprice,'utf-8','gbk');
        $carage = changeCode($carage,'utf-8','gbk');
        $bodytype = changeCode($bodytype,'utf-8','gbk');
        $kilometres = changeCode($kilometres,'utf-8','gbk');
        $transmission = changeCode($transmission,'utf-8','gbk');
        $message = changeCode($message,'utf-8','gbk');
    ...
    function post714(){
        ...
        if(!empty($cityStr) || !empty($suburb) || !empty($postcode)){
            $tmp .='<tr><th class="tit">城市:</th><td class="con">'.$cityStr." ".$suburb.' '.$postcode.'</td></tr>';
        }
        if(!empty($poster)){
            $tmp .='<tr><th class="tit">姓名:</th><td class="con">'.$poster.'</td></tr>';
        }
        if(!empty($educationStr)){
            $tmp .='<tr><th class="tit">最高学历:</th><td class="con">'.$educationStr.'</td></tr>';
        }
        if(!empty($ages)){
            $tmp .='<tr><th class="tit">年龄:</th><td class="con">'.$ages.'</td></tr>';
        }
        if(!empty($property)){
            $tmp .='<tr><th class="tit">工作性质:</th><td class="con">'.$property.'</td></tr>';
        }
        if(!empty($visaStr)){
            $tmp .='<tr><th class="tit">签证状态:</th><td class="con">'.$visaStr.'</td></tr>';
        }
        if(!empty($positionStr)){
            $tmp .='<tr><th class="tit">期望职位:</th><td class="con">'.$positionStr.'</td></tr>';
        }
        $tmp .='<tr><th class="tit">具体职位:</th><td class="con">&nbsp;</td></tr>';
        if(!empty($salary_formStr) || !empty($salaryStr)){
            $tmp .='<tr><th class="tit">期望薪资:</th><td class="con">'.$salary_formStr."/".$salaryStr.'</td></tr>';
        }
        if(!empty($tel)){
            $tmp .='<tr><th class="tit">联系电话:</th><td class="con">'.$tel.'</td></tr>';
        }
        if(!empty($worktime)){
            $tmp .='<tr><th class="tit">可工作时间:</th><td class="con">'.$worktime.'</td></tr>';
        }
        if(!empty($weixin)){
            $tmp .='<tr><th class="tit">微信:</th><td class="con">'.$weixin.'</td></tr>';
        }
        ...
        $suburb = changeCode($suburb,'utf-8','gbk');
        $postcode = changeCode($postcode,'utf-8','gbk');
        $position = changeCode($position,'utf-8','gbk');
        $salary_form = changeCode($salary_form,'utf-8','gbk');
        $salary = changeCode($salary,'utf-8','gbk');
        $ages = changeCode($ages,'utf-8','gbk');
        $education = changeCode($education,'utf-8','gbk');
        $property = changeCode($property,'utf-8','gbk');
        $visa = changeCode($visa,'utf-8','gbk');
        $worktime = changeCode($worktime,'utf-8','gbk');
        $message = changeCode($message,'utf-8','gbk');
        $poster = changeCode($poster,'utf-8','gbk');
        $tel = changeCode($tel,'utf-8','gbk');
        $weixin = changeCode($weixin,'utf-8','gbk');
    ...
    function post161(){
        ...
        if(!empty($typeidStr)){
            $tmp .='<tr><th class="tit">服务类别:</th><td class="con">'.$typeidStr.'</td></tr>';
        }
        if(!empty($positionStr)){
            $tmp .='<tr><th class="tit">具体职位:</th><td class="con">'.$positionStr.'</td></tr>';
        }
        if(!empty($company)){
            $tmp .='<tr><th class="tit">公司名称:</th><td class="con">'.$company.'</td></tr>';
        }
        if(!empty($property)){
            $tmp .='<tr><th class="tit">工作性质:</th><td class="con">'.$property.'</td></tr>';
        }
        if(!empty($visa)){
            $tmp .='<tr><th class="tit">签证状态:</th><td class="con">'.$visa.'</td></tr>';
        }
        if(!empty($experience)){
            $tmp .='<tr><th class="tit">经验要求:</th><td class="con">'.$experience.'</td></tr>';
        }
        if(!empty($educationStr)){
            $tmp .='<tr><th class="tit">学历要求:</th><td class="con">'.$educationStr.'</td></tr>';
        }
        if(!empty($p_num)){
            $tmp .='<tr><th class="tit">招聘人数:</th><td class="con">'.$p_num.'人</td></tr>';
        }
        if(!empty($superannua)){
            $tmp .='<tr><th class="tit">Superannuation:</th><td class="con">'.$superannua.'</td></tr>';
        }
        if(!empty($annualleave)){
            $tmp .='<tr><th class="tit">Annual leave:</th><td class="con">'.$annualleave.'</td></tr>';
        }
        if(!empty($salary_formStr) || !empty($salaryStr)){
            $tmp .='<tr><th class="tit">工资待遇:</th><td class="con">'.$salary_formStr.'/'.$salaryStr.'</td></tr>';
        }
        if(!empty($poster)){
            $tmp .='<tr><th class="tit">联系人:</th><td class="con">'.$poster.'</td></tr>';
        }
        ...
        $subject = changeCode($subject,'utf-8','gbk');
        $suburb = changeCode($suburb,'utf-8','gbk');
        $company = changeCode($company,'utf-8','gbk');
        $position = changeCode($position,'utf-8','gbk');
        $property = changeCode($property,'utf-8','gbk');
        $visa = changeCode($visa,'utf-8','gbk');
        $experience = changeCode($experience,'utf-8','gbk');
        $education = changeCode($education,'utf-8','gbk');
        $p_num = changeCode($p_num,'utf-8','gbk');
        $salary_form = changeCode($salary_form,'utf-8','gbk');
        $salary = changeCode($salary,'utf-8','gbk');
        $annualleave = changeCode($annualleave,'utf-8','gbk');
        $superannua = changeCode($superannua,'utf-8','gbk');
        $message = changeCode($message,'utf-8','gbk');
        $poster = changeCode($poster,'utf-8','gbk');
        $tel = changeCode($tel,'utf-8','gbk');
        $weixin = changeCode($weixin,'utf-8','gbk');
    ...
    function post142(){
        ...
        $keep_petStr = $paramAry['keep_pet'][$keep_pet];
        ...
        if(!empty($cityStr) || !empty($suburb)){
            $tmp .='<tr><th class="tit">所在城市:</th><td class="con">'.$cityStr." ".$suburb.'</td></tr>';
        }
        if(!empty($validityStr)){
            $tmp .='<tr><th class="tit">有效期:</th><td class="con">'.$validityStr.'</td></tr>';
        }
        if(!empty($address)){
            $tmp .='<tr><th class="tit">地址:</th><td class="con">'.$address.'</td></tr>';
        }
        if(!empty($house_fromStr)){
            $tmp .='<tr><th class="tit">来源:</th><td class="con">'.$house_fromStr.'</td></tr>';
        }
        if(!empty($iscityStr)){
            $tmp .='<tr><th class="tit">是否在City:</th><td class="con">'.$iscityStr.'</td></tr>';
        }
        if(!empty($house_typeStr)){
            $tmp .='<tr><th class="tit">类型:</th><td class="con">'.$house_typeStr.'</td></tr>';
        }
        if(!empty($house_roomStr) || !empty($house_toiletStr) || !empty($house_kuStr)){
            $tmp .='<tr><th class="tit">配置:</th><td class="con">'.$house_roomStr.'室'.$house_toiletStr.'卫'.$house_kuStr.'车库</td></tr>';
        }
        if(!empty($house_rents)){
            $tmp .='<tr><th class="tit">租金:</th><td class="con">'.$house_rents.'人</td></tr>';
        }
        if(!empty($rent_typeStr)){
            $tmp .='<tr><th class="tit">出租方式:</th><td class="con">'.$rent_typeStr.'</td></tr>';
        }
        if(!empty($in_date)){
            $tmp .='<tr><th class="tit">Annual 可入住时间:</th><td class="con">'.$in_date.'</td></tr>';
        }
        if(!empty($house_sexStr)){
            $tmp .='<tr><th class="tit">性别要求:</th><td class="con">'.$house_sexStr.'</td></tr>';
        }
        if(!empty($keep_petStr)){
            $tmp .='<tr><th class="tit">是否可养宠物:</th><td class="con">'.$keep_petStr.'</td></tr>';
        }
        if(!empty($house_equipmentStr)){
            $tmp .='<tr><th class="tit">配套设施:</th><td class="con">'.$house_equipmentStr.'</td></tr>';
        }
        if(!empty($bus_infoStr)){
            $tmp .='<tr><th class="tit">周边配套:</th><td class="con">'.$bus_infoStr.'</td></tr>';
        }
        if(!empty($poster)){
            $tmp .='<tr><th class="tit">联系人:</th><td class="con">'.$poster.'</td></tr>';
        }
        ...
        //写入专门的
        $subject = changeCode($subject,'utf-8','gbk');
        $validity = changeCode($validity,'utf-8','gbk');
        $suburb = changeCode($suburb,'utf-8','gbk');
        $address = changeCode($address,'utf-8','gbk');
        $house_from = changeCode($house_from,'utf-8','gbk');
        $iscity = changeCode($iscity,'utf-8','gbk');
        $house_type = changeCode($house_type,'utf-8','gbk');
        $house_room = changeCode($house_room,'utf-8','gbk');
        $house_hall = changeCode($house_hall,'utf-8','gbk');
        $house_toilet = changeCode($house_toilet,'utf-8','gbk');
        $house_balcony = changeCode($house_balcony,'utf-8','gbk');
        $house_ku = changeCode($house_ku,'utf-8','gbk');
        $house_rents = changeCode($house_rents,'utf-8','gbk');
        $rent_type = changeCode($rent_type,'utf-8','gbk');
        $in_date = changeCode($in_date,'utf-8','gbk');
        $house_sex = changeCode($house_sex,'utf-8','gbk');
        $house_equipment = changeCode($house_equipment,'utf-8','gbk');
        $bus_info = changeCode($bus_info,'utf-8','gbk');
        $message = changeCode($message,'utf-8','gbk');
        $poster = changeCode($poster,'utf-8','gbk');
        $tel = changeCode($tel,'utf-8','gbk');
        $weixin = changeCode($weixin,'utf-8','gbk');
        ...
        $sql = "insert into
        pre_forum_field_house_for_rent
        set
          city='".$city."',
          tid='".$tid."',
          validity='".$validity."',
          ...
          keep_pet='".$keep_pet."',
          house_toilet='".$house_toilet."',
          house_balcony='".$house_balcony."',
          house_ku='".$house_ku."',
          house_rents='".$house_rents."',
          rent_type='".$rent_type."',
          in_date='".$in_date."',
          house_sex='".$house_sex."',
          house_equipment='".addslashes(serialize($house_equipment))."',
          bus_info='".addslashes(serialize($bus_info))."',
          message='".$message."',
          poster='".$poster."',
          tel='".$tel."',
          weixin='".$weixin."',
    ...
    function update142(){
        ...
        $tmp .='<tr><td class="tit">性别要求:</td><td class="con2">'.$house_sexStr.'</td><td class="tit">是否可养宠物:</td><td class="con">'.$keep_petStr.'</td></tr>';
        $tmp .='<td class="tit">配套设施:</td><td class="con">'.$house_equipmentStr.'</td></tr><tr><td class="tit">周边配套:</td><td class="con2">'.$bus_infoStr.'</td>';
        $tmp .='<td class="tit">联系人:</td><td class="con">'.$poster.'</td></tr><tr><td class="tit">电话:</td><td class="con2">'.$tel.'</td><td class="tit">微信:</td><td class="con">'.$weixin.'</td></tr>';
        $tmp .='<tr><td class="tit">QQ:</td><td class="con2">'.$qq.'</td><td class="tit">邮箱:</td><td class="con">'.$email.'</td></tr>';
        $tmp .="</table></div>";
        $tmp .="<p>".$nomalMessage."</p>";
        ...
        $sql = "
        update pre_forum_field_house_for_rent
        set
        city='".$city."',
        validity='".$validity."',
        suburb='".$suburb."',
    ...
    function post680(){
        ...
        if(!empty($rent_typeStr)){
            $tmp .='<tr><th class="tit">求租方式:</th><td class="con">'.$rent_typeStr.'</td></tr>';
        }
        if(!empty($poster)){
            $tmp .='<tr><th class="tit">联系人:</th><td class="con">'.$poster.'</td></tr>';
        }
        ...
        if(!empty($qq)){
            $tmp .='<tr><td class="tit">QQ:</td><td class="con2">'.$qq.'</td></tr>';
        }
        if(!empty($email)){
            $tmp .='<tr><td class="tit">邮箱:</td><td class="con">'.$email.'</td></tr>';
        }
        $tmp .="</table>";
        $tmp .="<p>".$nomalMessage."</p>";
    ...
    function post305(){
        ...
        if(!empty($description)){
            $tmp .='<tr><th class="tit">房屋特色:</th><td colspan="3">'.$description.'</td></tr>';
        }
        if(!empty($price)){
            $tmp .='<tr><th class="tit">出售价格:</th><td class="con">'.$price.'</td></tr>';
        }
        if(!empty($house_typeStr)){
            $tmp .='<tr><th class="tit">类型:</th><td class="con">'.$house_typeStr.'</td></tr>';
        }
        if(!empty($house_roomStr) || !empty($house_hallStr) || !empty($house_toiletStr)){
            $tmp .='<tr><th class="tit">配置:</th><td class="con">'.$house_roomStr.'室'.$house_hallStr.'厅'.$house_toiletStr.'卫'.$house_balconyStr.'阳台'.$house_kuStr.'车库</td></tr>';
        }
        if(!empty($showtime)){
            $tmp .='<tr><th class="tit">看房时间:</th><td class="con">'.$showtime.'人</td></tr>';
        }
        if(!empty($readyhouseStr)){
            $tmp .='<tr><th class="tit">是否现房:</th><td class="con">'.$readyhouseStr.'</td></tr>';
        }
        if(!empty($allowforeignerStr)){
            $tmp .='<tr><th class="tit">海外人士购买:</th><td class="con">'.$allowforeignerStr.'</td></tr>';
        }
        if(!empty($poster)){
            $tmp .='<tr><th class="tit">联系人:</th><td class="con">'.$poster.'</td></tr>';
        }
        ...
        $subject = changeCode($subject,'utf-8','gbk');
        $validity = changeCode($validity,'utf-8','gbk');
        $description = changeCode($description,'utf-8','gbk');
        $house_type = changeCode($house_type,'utf-8','gbk');
        $suburb = changeCode($suburb,'utf-8','gbk');
        $address = changeCode($address,'utf-8','gbk');
        $iscity = changeCode($iscity,'utf-8','gbk');
        $price = changeCode($price,'utf-8','gbk');
        $showtime = changeCode($showtime,'utf-8','gbk');
        $readyhouse = changeCode($readyhouse,'utf-8','gbk');
        $house_room = changeCode($house_room,'utf-8','gbk');
        $house_hall = changeCode($house_hall,'utf-8','gbk');
        $house_toilet = changeCode($house_toilet,'utf-8','gbk');
        $house_balcony = changeCode($house_balcony,'utf-8','gbk');
        $house_ku = changeCode($house_ku,'utf-8','gbk');
        $allowforeigner = changeCode($allowforeigner,'utf-8','gbk');
        $message = changeCode($message,'utf-8','gbk');
        $poster = changeCode($poster,'utf-8','gbk');
        $tel = changeCode($tel,'utf-8','gbk');
        $weixin = changeCode($weixin,'utf-8','gbk');
    ...
    function post681(){
        ...
        if(!empty($iscityStr)){
            $tmp .='<tr><th class="tit">是否City:</th><td class="con">'.$iscityStr.'</td></tr>';
        }
        if(!empty($house_typeStr)){
            $tmp .='<tr><th class="tit">房屋类型:</th><td class="con">'.$house_typeStr.'</td></tr>';
        }
        if(!empty($house_position)){
            $tmp .='<tr><th class="tit">目标区域</th><td class="con">'.$house_position.'</td></tr>';
        }
        if(!empty($poster)){
            $tmp .='<tr><th class="tit">联系人:</th><td class="con">'.$poster.'</td></tr>';
        }
        ...
        $qq = changeCode($qq,'utf-8','gbk');
        $email = changeCode($email,'utf-8','gbk');
        $house_position = changeCode($house_position,'utf-8','gbk');
    ...
    function post304(){
        ...
        if(!empty($markettypeStr)){
            $tmp .='<tr><th class="tit">求购/出售:</th><td class="con">'.$markettypeStr.'</td></tr>';
        }
        if(!empty($deliveryStr)){
            $tmp .='<tr><th class="tit">送货:</th><td class="con">'.$deliveryStr.'</td></tr>';
        }
        if(!empty($fromtypeStr)){
            $tmp .='<tr><th class="tit">来源</th><td class="con">'.$fromtypeStr.'</td></tr>';
        }
        if(!empty($poster)){
            $tmp .='<tr><th class="tit">联系人:</th><td class="con">'.$poster.'</td></tr>';
        }
        ...
        $suburb = changeCode($suburb,'utf-8','gbk');
        $postcode = changeCode($postcode,'utf-8','gbk');
        $typeid = changeCode($typeid,'utf-8','gbk');
        $delivery = changeCode($delivery,'utf-8','gbk');
        $fromtype = changeCode($fromtype,'utf-8','gbk');
        $markettype = changeCode($markettype,'utf-8','gbk');
        $price = changeCode($price,'utf-8','gbk');
        $message = changeCode($message,'utf-8','gbk');
        $poster = changeCode($poster,'utf-8','gbk');
        $tel = changeCode($tel,'utf-8','gbk');
    ...
    function post89(){
        ...
        if(!empty($price)){
            $tmp .='<tr><th class="tit">价格</th><td class="con">'.$price.'</td></tr>';
        }
        if(!empty($poster)){
            $tmp .='<tr><th class="tit">联系人:</th><td class="con">'.$poster.'</td></tr>';
        }
    ...
    function post716(){
        ...
        if(!empty($hangye)){
            $tmp .='<tr><th class="tit">所属行业:</th><td class="con">'.$hangye.'</td></tr>';
        }
        if(!empty($company_name)){
            $tmp .='<tr><th class="tit">公司名称:</th><td class="con">'.$company_name.'</td></tr>';
        }
        if(!empty($company_address)){
            $tmp .='<tr><th class="tit">公司地址:</th><td class="con">'.$company_address.'</td></tr>';
        }
        if(!empty($price)){
            $tmp .='<tr><th class="tit">价格</th><td class="con">'.$price.'</td></tr>';
        }
        ...
        $subject = changeCode($subject,'utf-8','gbk');
        $suburb = changeCode($suburb,'utf-8','gbk');
        $hangye = changeCode($hangye,'utf-8','gbk');
        $company_name = changeCode($company_name,'utf-8','gbk');
        $company_address = changeCode($company_address,'utf-8','gbk');
        $price = changeCode($price,'utf-8','gbk');
        $message = changeCode($message,'utf-8','gbk');
        $poster = changeCode($poster,'utf-8','gbk');
        $tel = changeCode($tel,'utf-8','gbk');
    ...
    function post679(){
        ...
        if(!empty($schoolStr)){
            $tmp .='<tr><th class="tit">学校:</th><td class="con">'.$schoolStr.'</td></tr>';
        }
        if(!empty($xuqiuStr)){
            $tmp .='<tr><th class="tit">出售/求购:</th><td class="con">'.$xuqiuStr.'</td></tr>';
        }
        if(!empty($typeidStr)){
            $tmp .='<tr><th class="tit">类别:</th><td class="con">'.$typeidStr.'</td></tr>';
        }
        ...
        $subject = changeCode($subject,'utf-8','gbk');
        $suburb = changeCode($suburb,'utf-8','gbk');
        $school = changeCode($school,'utf-8','gbk');
        $xuqiu = changeCode($xuqiu,'utf-8','gbk');
        $typeid = changeCode($typeid,'utf-8','gbk');
        $price = changeCode($price,'utf-8','gbk');
        $message = changeCode($message,'utf-8','gbk');
        $poster = changeCode($poster,'utf-8','gbk');
    ...
    function post651(){
        ...
        if(!empty($company_name)){
            $tmp .='<tr><th class="tit">公司名字:</th><td class="con">'.$company_name.'</td></tr>';
        }
        if(!empty($company_address)){
            $tmp .='<tr><th class="tit">公司地址:</th><td class="con">'.$company_address.'</td></tr>';
        }
        if(!empty($company_website)){
            $tmp .='<tr><th class="tit">公司网站:</th><td class="con">'.$company_website.'</td></tr>';
        }
        if(!empty($company_info)){
            $tmp .='<tr><th class="tit">公司介绍:</th><td class="con">'.$company_info.'</td></tr>';
        }
        if(!empty($company_service)){
            $tmp .='<tr><th class="tit">服务内容:</th><td class="con">'.$company_service.'</td></tr>';
        }
        if(!empty($company_area)){
            $tmp .='<tr><th class="tit">服务区域:</th><td class="con">'.$company_area.'</td></tr>';
        }
        if(!empty($poster)){
            $tmp .='<tr><th class="tit">联系人:</th><td class="con">'.$poster.'</td></tr>';
        }
        ...
        $company_name = changeCode($company_name,'utf-8','gbk');
        $company_address = changeCode($company_address,'utf-8','gbk');
        $company_website = changeCode($company_website,'utf-8','gbk');
        $company_info = changeCode($company_info,'utf-8','gbk');
        $company_service = changeCode($company_service,'utf-8','gbk');
        $company_area = changeCode($company_area,'utf-8','gbk');
        $message = changeCode($message,'utf-8','gbk');
        $poster = changeCode($poster,'utf-8','gbk');
        $tel = changeCode($tel,'utf-8','gbk');
        $weixin = changeCode($weixin,'utf-8','gbk');
    ...
    function updateThread($tid,$pid,$subject,$message){
        ...
        /* 2017.05.19 edit by allen qu  strip_tags把所有的标签都屏蔽了, 坑坑坑!!!!!!! */
        //$message = strip_tags($message);
        $subject = stripslashes($subject);
        $subject = addslashes($subject);
        $messageNoSlashes = stripslashes($message);
        /* 2017.05.19 edit by allen qu 新加  */
        ...
        $firstPic = '';
        /* 2017.05.25 edit by allen qu 将pre_forum_thread_pic表里的数据也删除 */
        $this->Db->query("DELETE FROM pre_forum_thread_pic where tid=".$tid);


        /* edit by neek li 调用PC端postnew接口 */
        $ip = client_ip();
        $threadInfo = $this->Db->once_fetch_assoc("select * from pre_forum_thread where tid=".$tid);
        $res = postNew($tid,'modify',$ip,MEMBER_ID,$threadInfo['fid']);
        file_put_contents('fenleixiugai.txt',var_export($res,true));
    ...
    function postThread($fid,$subject,$message,$dateline,$typeid=0){
        ...
        $subject = addslashes($subject);
        $messageNoSlashes = stripslashes($message);
        $messageNoSlashes = strip_tags($messageNoSlashes,"<p><th><br><div><table><tr><td><span><font><img>");
        $message = addslashes($messageNoSlashes);
        $message .= '<p style="color:blue;margin-top:30px;font-weight:bolder;text-align:right;">发自<a href="https://m.yeeyi.com/mobile/client/index.php">亿忆澳洲APP</a></p>';
        $subject = changeCode($subject,'utf-8','gbk');
        $message = changeCode($message,'utf-8','gbk');
        ...
        $ip = client_ip();
        //写入了
        $status = ($fid==277) ? 34 : 32;
        $result = $this->Db->query("insert into `pre_forum_thread` (`fid`,`typeid`,`author`,`authorid`,`subject`,`attachment`,`dateline`,`lastpost`,`lastposter`,`views`,`status`) values ('".$fid."','".$typeid."','".$username."','".MEMBER_ID."','".$subject."','','".$dateline."','".$dateline."','".$username."',0,'".$status."')");
        ...
        //添加发布时间
        //$this->Db->query("insert into `pre_forum_thread_refresh` (`tid`,`refresh`) values ('".$tid."','".$dateline."')");

        /* edit by neek li 调用PC端postnew接口 */
        $res = postNew($tid,'new',$ip,MEMBER_ID,$fid);
