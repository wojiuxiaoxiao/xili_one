    ...
    function relatedThread($params, $tid)
	{
		...
		if ($fid == 142 && ($suburb && $house_type)) {
			...
			$items = $this->Db->fetch_all_assoc(
				'select  142 as fid, b.tid, b.subject, b.lastpost, c.pic, suburb, house_type, house_toilet, house_room, house_rents, house_ku, in_date, b.displayorder
				...
				on b.tid = c.tid
                where b.displayorder=0 and pre_forum_field_house_for_rent.tid != '.$tid .'
		...
		if ($fid == 161 && $position && $property) {
			...
			$sql = "select 161 as fid, b.tid, b.subject, b.lastpost, c.pic, suburb, position, property, experience, b.lastpost, b.displayorder
			from professional_job_fire_detail
			left join pre_forum_thread as b
			on professional_job_fire_detail.tid =b.tid
			left join pre_forum_thread_pic as c
			on b.tid = c.tid
            where b.displayorder=0 and professional_job_fire_detail.tid != ".$tid ."
			 ".$select." order by b.lastpost desc limit ".$limit;
        ...
        if ($fid == 304 && $typeid && isset($delivery)) {
            ...
            $sql = "select 304 as fid, b.tid, b.subject, b.lastpost, c.pic, suburb,pre_forum_field_market.price,poster, b.lastpost, b.displayorder, case pre_forum_field_market.delivery when '1' then '送货' when '2' then '自取' else '' END AS delivery
			from pre_forum_field_market
			left join pre_forum_thread as b
			on pre_forum_field_market.tid =b.tid
			left join pre_forum_thread_pic as c
			on b.tid = c.tid
            where b.displayorder=0 and pre_forum_field_market.tid != ".$tid ."
			 ".$select." order by b.lastpost desc limit ".$limit;
            $sql = changeCode($sql, 'utf-8', 'gbk');
        ...
        if ($fid == 291 && $carmake && isset($MinPrice)) {
            ...
            $sql = "select 291 as fid, b.tid, b.subject, b.lastpost, c.pic, suburb,professional_car_sale_detail.price,poster,b.displayorder, b.lastpost,CASE professional_car_sale_detail.kilometres when '0' then '' else CONCAT(professional_car_sale_detail.kilometres,'公里') END AS kilometres
			from professional_car_sale_detail
			left join pre_forum_thread as b
			on professional_car_sale_detail.tid =b.tid
			left join pre_forum_thread_pic as c
			on b.tid = c.tid
            where b.displayorder=0 and  professional_car_sale_detail.tid != ".$tid ."
			 ".$select." order by b.lastpost desc limit ".$limit;
            $sql = changeCode($sql, 'utf-8', 'gbk');
...
//        $s3[] = array('所在地区',$cityStr." ".$suburb." ".$postcode);
    ...
    function viewThread161($tid){
        ...
        $education = $professionInfo['education'];
        /* add by chen at 2017-11-09 */
        if($professionInfo['p_num'] == 0){
            $p_num = '';
        }else{
...
//        $s3[] = array('车库',$house_kuStr);
        ...
        $s3[] = array('可入住时间',date('Y-m-d',strtotime($in_date)));
