...
//	    $keywords = changeCode($keywords, 'utf-8', 'gbk');
	...
	function getNewsList(){
		...
		if($cid>0){
			...
			$newsAry = $this->Db->fetch_all_assoc("select aid,title,pic,pic2,pic3,description,pubdate,froms,tourl,aid as orderid,lighten from news_article where ".$whereAid." cid in(".implode(',',$cidAry).") order by pubdate desc limit 0,".$amount);
		}
		else{
            if($startAid == 0){
                ...
                if(!empty($NewsList_from_cache)){
                    ...
                    $newsAry = $this->Db->fetch_all_assoc("select aid,title,pic,pic2,pic3,description,pubdate,froms,tourl,essence as orderid,lighten from news_article where delstate=0 and essence>0 order by essence desc limit 0,".$amount);
                ...
                $newsAry = $this->Db->fetch_all_assoc("select aid,title,pic,pic2,pic3,description,pubdate,froms,tourl,essence as orderid,lighten from news_article where delstate=0 and essence<$startAid and essence>0 order by essence desc limit 0,".$amount);
            }
		}
		...
		if ($startAid == 0 && in_array($cid,$showAd)) {
            ...
            if ($news_ad['status'] == 1){
				...
				$i = 0;
				if($this->Config['article_default']['news_index']){
					$arr[] = $this->Config['article_default']['news_index'];
					add_show($this->Config['article_default']['news_index']['ad_id']);
					array_splice($tmpAry, 0, 0, $arr);
					$i++;
					unset($arr);
				}
				if($this->Config['article_default']['news_3']){
					$arr[] = $this->Config['article_default']['news_3'];
					add_show($this->Config['article_default']['news_3']['ad_id']);
					array_splice($tmpAry, $i+3, 0, $arr);
					$i++;
					unset($arr);
				}
				if($this->Config['article_default']['news_9']){
					$arr[] = $this->Config['article_default']['news_9'];
					add_show($this->Config['article_default']['news_9']['ad_id']);
					array_splice($tmpAry, $i+9, 0, $arr);
					$i++;
					unset($arr);
				}
				if($this->Config['article_default']['news_12']){
					$arr[] = $this->Config['article_default']['news_12'];
					add_show($this->Config['article_default']['news_12']['ad_id']);
					array_splice($tmpAry, $i+12, 0, $arr);
					$i++;
					unset($arr);
				}
