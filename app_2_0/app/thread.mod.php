...
//个人收藏
	...
	function refreshThread(){
		...
		/* 2017.05.11 edit by allen qu  顶贴时间间隔改为15分钟*/
		//$nexttime = time() + mt_rand(8,30)*60;
		if(time()<$lastFresh){
			$return['time'] = ceil(($lastFresh - time()) / 60);
			$return['status'] = 5701;
			$this->outjson($return);
		...
		/* edit by neek li 2017.08.07 顶贴不扣除2YB */
		$this->Db->query("update pre_forum_thread set lastpost=".time()." where tid=".$tid);
//		$otherInfo = $this->Db->once_fetch_assoc("select * from pre_common_member_count where uid=".MEMBER_ID);

//		if($otherInfo['extcredits2'] >= 2) {
//
//			if($threadInfo['authorid'] == MEMBER_ID){
//				$this->Db->query("update pre_forum_thread set lastpost=".time()." where tid=".$tid);
//				$this->Db->query("update pre_common_member_count set extcredits2 = extcredits2 - 2 where uid=".MEMBER_ID);//扣除2YB
//				$return['status'] = 2701;
//			}
//			else{
//				$return['status'] = 5703;
