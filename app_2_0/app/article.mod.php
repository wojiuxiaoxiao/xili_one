...
//			if($immedia_temp==3 && $cid==0 && $startAid==0){//直接在此处写静态数据
//				$ad = array();
//				$ad['title'] = '卡斯丽Castle楼王惊艳悉尼 连创二项历史纪录';
//				$ad['pic'] = 'https://apps.yeeyi.com/public/splash/market.jpg';
//				$ad['froms'] = '悉尼卡斯丽';
//				$ad['url'] = 'http://www.yeeyi.com/news/index.php?app=home&act=article&aid=179789';
//				$ad['pic_style'] = 'large';
//				$ad['isAds'] = 1;
...
//			}
			...
			if($immedia_temp==8 && $cid==0){//无广告8，有广告7
				if(isset($topicAry[1])) {
					$tmp = array();
					$tmp['isNews'] = 0;
	...
	function getNewsContent(){
		...
		$newsAry['content'] = strip_tags($newsAry['content'],'<a><p><br><div><img><font><span><iframe><video><audio><embed>');
		//处理内容中的图片
		...
		*/
		...
		if(MEMBER_ID<1){
			...
			if(time()<$lastArticleReply){
				...
				$return['message'] = changeCode('30秒内不得重复回复同一新闻', 'utf-8', 'gbk');
				$this->outjson($return);
