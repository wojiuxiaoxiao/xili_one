...
//        $message = changeCode($message,'utf-8','gbk');
	...
	function topicNewList(){
		...
		$topicArray = $this->Db->fetch_all_assoc("
						select pre_common_member.groupid,pre_forum_thread.tid,pre_forum_thread.lastpost,pre_forum_thread.views,pre_forum_thread.author,pre_forum_thread.fid,pre_forum_thread.subject,pre_forum_thread.author,pre_forum_thread.authorid,pre_forum_thread.dateline,replies,'0'
						as likes,pic,lastpost,pre_forum_thread.typeid
						...
						order by pre_forum_thread.dateline
						desc
						limit 150"
		);
...
//				$tmp['ceshi'] = $postlast['dateline'];
			...
			if($start>0){
				if ($typeid == 0) {
					$listTmp = $this->Db->fetch_all_assoc("
						select pre_forum_thread.tid,pre_forum_thread.subject,pre_forum_thread.author,pre_forum_thread.authorid,pre_forum_thread.dateline,replies,'0'
						as likes,pic,lastpost,pre_forum_thread.typeid
						from pre_forum_thread
						left join pre_forum_thread_pic
						on pre_forum_thread.tid=pre_forum_thread_pic.tid
						where fid in(".implode(',',$fidAry).")
						and displayorder>-1
						and pre_forum_thread.lastpost<$start
	...
	function postTopic(){
		...
		*/
		...
		$return['yb_message'] = '发帖成功, 金钱+40YB';
		/* edit end */
	...
	function replyTopic(){
		...
		*/
		...
		$return['yb_message'] = '评论成功, 金钱+12YB';
