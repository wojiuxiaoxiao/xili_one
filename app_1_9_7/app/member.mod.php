...
//		    delVar($keyCode);
    ...
    function sysNotice(){
        ...
        if($start>0){
            $list = $this->Db->fetch_all_assoc("select id,authorid,author,note,dateline from `pre_home_notification` where uid=".MEMBER_ID." and  `type`='post' and id<$start order by id desc limit $amount");
        }
        else{
            $list = $this->Db->fetch_all_assoc("select id,authorid,author,note,dateline from `pre_home_notification` where uid=".MEMBER_ID."  and  `type`='post' order by id desc limit $amount");
        }
        foreach($list as $k=>$v){
...
//		}
		...
		//修改用户名
		$username = changeCode($name,'utf-8','gbk');
		$checkId = $this->Db->once_fetch_assoc("select uid from centery_main.uc_members WHERE username='".$username."'");
		if($checkId['uid']){
			$return['status'] = 1;
	  ...
	  $amount = intval($this->Post['amount']);
		...
		if($type == 2){
			...
			if($start>0){
				...
				SELECT author,authorid,replies,'0' as likes,pre_forum_forum.name,pre_forum_thread.fid,pre_forum_thread.tid,subject,dateline
			...
			foreach($threadList as $k=>$thread)
			{
				$picAry = array();
				//判断tabid
				$tableid = getattachtableid($threadList[$k]['tid']);
				$attTableName = "pre_forum_attachment_".$tableid;
				$attachAry = $this->Db->fetch_all_assoc("select attachment from $attTableName where tid=".$threadList[$k]['tid']." order by aid asc limit 9");
				foreach($attachAry as $atta){
					$picAry[] = "http://www.yeeyi.com/bbs/data/attachment/forum/".$atta['attachment'];
				}
				$threadList[$k]['pic'] = isset($picAry[0]) ? $picAry[0]:"";
			}
		}
		else{
			if($start>0){
...
//		$inmail = $inmail===false ? $inmail : 1;
	 ...
	 * @获取图片
	 */
	public function getPicList($tid, $type = 1)
	{
		$picAry = array();
		if ($tid > 0) {
			//判断tabid
