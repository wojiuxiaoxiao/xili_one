...
//        $message = changeCode($message,'utf-8','gbk');
	...
	function topicNewList(){
        ...
        $fidguanzhu = array(15,36,92,318,319,619,646);//热门
        ...
        //Tony 20180313 先去缓存读取

        $cache_fidArr = implode('_',$fidArr);
        $topicArray = array();
        $topicArray_from_cache = getVar('topicNewList_cache_'.$start.'_'.$amount.'_'.$cache_fidArr);
        if(!empty($topicArray_from_cache)){
            $topicArray = $topicArray_from_cache;
        }else{

            //Tony 20180313 不再关联pre_common_member表, 也不再关联pre_forum_post表
            ...
            //把从数据库得到的结果存在缓存, 180秒
            if(!empty($topicArray)){
                setVar('topicNewList_cache_'.$start.'_'.$amount.'_'.$cache_fidArr, $topicArray, 180);
            }
		...
		if(is_array($topicArray)){
			foreach($topicArray as $t){
                ...
                //Tony 20180313 不再提取 $groupInfo 信息
				...
				$tmp['isLike'] = 0;
				if($uid>0){
					$checkFavoriate = $this->Db->once_fetch_assoc("select * from app_post_like where uid=".$uid." and id=".$t['tid']." and idtype='tid'");
				...
				$tmp['bigpic'] = $this->getPicList($t['tid'],2);
                ...
                $tmp['group_name'] = '';  //Tony 20180313 把group_name直接返回为空字符串
		...
		/* add topic(find) advertise */
		if ($page == 1){
            $topic_ad =  get_ad('find');
            if ($topic_ad['status'] == 1){
                $advertises = $topic_ad['data'];
                $i = 0;
                if ($advertises['find_3']){
                    $arr[] = $advertises['find_3'];
					add_show($advertises['find_3']['ad_id']);
                    array_splice($return['listAry'], $i+3, 0, $arr);
                    $i++;
                    unset($arr);
                }
                if ($advertises['find_9']){
                    $arr[] = $advertises['find_9'];
					add_show($advertises['find_9']['ad_id']);
                    array_splice($return['listAry'], $i+9, 0, $arr);
                    $i++;
                    unset($arr);
                }
                if ($advertises['find_13']){
                    $arr[] = $advertises['find_13'];
					add_show($advertises['find_13']['ad_id']);
                    array_splice($return['listAry'], $i+13, 0, $arr);
                    $i++;
                    unset($arr);
                }

				if ($advertises['find_19']){
					$arr[] = $advertises['find_19'];
					add_show($advertises['find_19']['ad_id']);
					array_splice($return['listAry'], $i+19, 0, $arr);
					$i++;
					unset($arr);
				}
            }
		...
		if ($page == 2){
			...
			if ($topic_ad['status'] == 1){
				$advertises = $topic_ad['data'];
				$i = 0;
				if ($advertises['find_25']){
					$arr[] = $advertises['find_25'];
					add_show($advertises['find_25']['ad_id']);
					array_splice($return['listAry'], $i+4, 0, $arr);
					$i++;
					unset($arr);
				}
			}
		}
        /* end topic advertise add */
	...
	function topicList(){
		...
		if($fid>0){
			...
			foreach($typeAryTmp as $k=>$type){
				if($type['name']=='问答'){
					unset($typeAryTmp[$k]);
					array_unshift( $typeAryTmp, $type);
				}
			}
			array_unshift($typeAryTmp,  array('name' =>'全部', 'typeid'=>0 ));
			...
			if($start>0){
				...
				if ($typeid == 0) {
					$listTmp = $this->Db->fetch_all_assoc("
						...
						and b.status != 1
						and displayorder>-1    
						order by pre_forum_thread.lastpost
						desc
						limit $amount"
					);//order by pre_forum_thread_refresh.refresh
				}else{
			...
			if(is_array($listTmp)){
				foreach($listTmp as $t){
					...
					if($t['c_tid']>0){
						...
						$t['authorid'] = 5122650;
                        $replyCount = $this->Db->once_fetch_assoc("SELECT COUNT(*) as counts from news_comment where aid=".$t['c_aid']." and del_status=0");
                        $tmp['replies'] = $replyCount['counts'];
					}
					$tmp['address'] = changeCode('火星网友','utf-8','gbk');
					$tmp['author'] = $t['author'];
					$tmp['authorid'] = $t['authorid'];
	...
	function replyTopic(){
		...
		*/
		...
		//只有当前用户是发帖人的情况下,才增加红点数
		$threadInfo = $this->Db->once_fetch_assoc("select tid from pre_forum_thread where tid=".$tid." and authorid=".$this->G['uid']);
		if($threadInfo){
...
//        echo json_encode($return);
	...
	function postLike(){
		...
		if(intval($this->Post['uid'])>0){
			$selectUid = intval($this->Post['uid']);
			$insertUid = intval($this->Post['uid']);
		}else{
			$selectUid = 10000000009;
			$insertUid = 0;
		}
		$id = intval($this->Post['id']);
		$type = trim($this->Post['type']);//tid:帖子  aid:新闻  tid_rp:帖子回帖    aid_p新闻评论

		$status = $this->Db->once_fetch_assoc("select * from yeeyico_new.app_post_like where idtype='".$type."' and uid=".$selectUid." and id=".$id);
		...
		$this->Db->query("INSERT INTO yeeyico_new.app_post_like values('',$id,'$type',$insertUid)");
		$count = $this->Db->once_fetch_assoc("select count(*) as num from yeeyico_new.app_post_like where idtype='".$type."'and id=".$id." group by id;");
		$return['status'] = 0;
		$return['likeNum'] = intval($count['num']);
		$this->outjson($return);
	...
	function getReply(){
		...
		if($isForum){
			if($type == 'hot'){
				$reply = $this->getThreadReply($tid,1,5);
			}
			else{
				$reply = $this->getThreadReply($tid,$index,$amount);
			}
		}
		else{
	 ...
	 */
	function getThreadReply($tid,$index,$amount){

		//查出置頂帖推到數組中
		$headerSql = "select p.message, p.author, p.authorid, m.groupid,p.pid, p.dateline,k.dateline as kline, l.location as address
			from pre_forum_poststick as k
			left join pre_forum_post_location as l
			on k.pid = l.pid
			left join pre_forum_post as p
			on k.pid = p.pid
		...
		$headPost = $this->Db->fetch_all_assoc($headerSql);
		$headerPid = array();
		foreach($headPost as $header){
			$headerPid[] = $header['pid'];
		}

		if($index==1){
			$amount = $amount>count($headerPid)?$amount-count($headerPid):$amount;
		}
		$andIn = count($headerPid)>0 ? implode(',',$headerPid) : '0';
		$start = $amount*($index-1);
		...
		if($index==1){
			foreach($headPost as $headerList){
				array_unshift( $replyTmp, $headerList);
			}
		}
...
//            }
		...
		if(count($upPid)>0){
			...
			$upInfoAry = array();
			foreach($upPid as $pk=>$pv){
				$upInfoAry[] = $this->Db->once_fetch_assoc("select p.pid, p.message, p.author, p.authorid, m.groupid, p.dateline, l.location as address
				from pre_forum_post as p
				left join pre_forum_post_location as l
				on p.pid = l.pid
				left join pre_common_member as m
	...
	function doReplyThread($tid, $uppid, $message, $location, $geolocation,$uid=0,$fromuid){   
		...
		if($uppid>0){
			...
			$upReplyInfo['message'] = preg_replace("/\[.*\]/isU",'',$upReplyInfo['message']);
			$upReplyInfo['message'] = addslashes($upReplyInfo['message']);
