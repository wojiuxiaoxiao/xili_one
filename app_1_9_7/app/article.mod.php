...
//	    $keywords = changeCode($keywords, 'utf-8', 'gbk');
    ...
    function searchNews(){
        ...
        $topicAry = $this->Db->fetch_all_assoc("select topic_id,topic_name,pic1,pic2,pic3,total_nums from topic_list where del_status=0 and topic_name like '%$keywords%' order by topic_id desc limit " . $amount * ($page - 1) . "," . $amount);
        ...
        foreach ($newsAry as $news_key => $news) {
			...
			$newsCommments = $this->Db->once_fetch_assoc("SELECT COUNT(*) as counts from news_comment where aid=".$news['aid']." and del_status=0");
			$news['counts'] = $newsCommments['counts'];
            $tmpAry[] = $news;
        }
	...
	function getNewsList(){
        ...
        $amount = ($startAid==0 && $cid==0) ? ($amount-4) : $amount;
		...
		if($startAid == 0){  //hotNews放到这里，只有加载初始列表时调用hotnews置顶，数量=2条
            ...
            $hotNews = array();
            $hotNews_of_NewsList_cache = getVar('hotNews_of_NewsList_cache');

            if(!empty($hotNews_of_NewsList_cache)){
                $hotNews = $hotNews_of_NewsList_cache;
            }else{

                $hotNews = $this->Db->fetch_all_assoc("select * from news_article where headline>0 order by headline desc limit 0,2");

                if(!empty($hotNews)){
                    setVar('hotNews_of_NewsList_cache', $hotNews, 180);
                }
            }
		 ...
		 * 置顶、分类配置、两个热门
		 * 新闻列表固定显示16条
		 * 然后array_unshift把置顶、分类配置、两个热门分别推进数组
		 */
		$hotNewss = $this->Db->fetch_all_assoc("select aid,title,pic,pic2,pic3,description,headline,pubdate,froms,tourl,essence as orderid,lighten from news_article where headline>0 order by headline desc limit 0,3");
		$hot_aids = array();
		foreach ($hotNewss as $hot_aid){
            $hot_aids[] = $hot_aid['aid'];
        }
        $hot_aids = implode(',',$hot_aids);

		$news_forms = $this->Config['news_form'];
		...
		if($cid>0){
			...
			$newsAry = $this->Db->fetch_all_assoc("select aid,title,pic,pic2,pic3,description,pubdate,froms,tourl,aid as orderid,lighten from news_article where ".$whereAid." cid in(".implode(',',$cidAry).") and aid not in (".$hot_aids.") order by pubdate desc limit 0,".$amount);
		}
		else{
            if($startAid == 0){

                //Tony20180321 缓存newslist
                $NewsList_from_cache = getVar('NewsList_cache_0_'.$amount);
                if(!empty($NewsList_from_cache)){
                    $newsAry = $NewsList_from_cache;
                }else{
                    $newsAry = $this->Db->fetch_all_assoc("select aid,title,pic,pic2,pic3,description,pubdate,froms,tourl,essence as orderid,lighten from news_article where delstate=0 and essence>0 and aid not in (".$hot_aids.") order by essence desc limit 0,".$amount);

                    if(!empty($newsAry)){
                        setVar('NewsList_cache_0_'.$amount, $newsAry, 150);   //保存2分半到缓存
                    }
                }

            }
            else{
                $newsAry = $this->Db->fetch_all_assoc("select aid,title,pic,pic2,pic3,description,pubdate,froms,tourl,essence as orderid,lighten from news_article where delstate=0 and essence<$startAid and essence>0 and aid not in (".$hot_aids.") order by essence desc limit 0,".$amount);
            }
		}

//		if($cid==0 && $startAid==0){   //修改取消置顶和热门
//            //开始 然后array_unshift
//            $hotNewss[2]['isHot'] = 1;
//            unset($hotNewss[2]['isNews']);
//            array_unshift($newsAry,$hotNewss[2]);
//
//            $hotNewss[1]['isHot'] = 1;
//            unset($hotNewss[1]['isNews']);
//            array_unshift($newsAry,$hotNewss[1]);
//
//            $hotNewss[0]['isHot'] = 1;
//            unset($hotNewss[1]['isNews']);
//            array_unshift($newsAry,$hotNewss[0]);
//
//            $hotNewss[0]['isTop'] = 1;
//            unset($hotNewss[1]['isNews']);
//            array_unshift($newsAry,$hotNewss[0]);
//        }
		...
		foreach($newsAry as $news_key=>$news){
			$newsCommments = $this->Db->once_fetch_assoc("SELECT COUNT(*) as counts from news_comment where aid=".$news['aid']." and del_status=0");
			$news['counts'] = $newsCommments['counts'];
			$news['pubdate4'] = date("Y-m-d H:i:s",$news['pubdate']);
			...
			if(strlen($news['tourl'])>5){
				...
				$news['outlink'] = $this->doLink($news['tourl']);
				$news['froms'] = $news['outlink']['author'];
				if($news['outlink']['tid']>0 || $news['outlink']['cid']>0){
					$tid = ($news['outlink']['tid'] > $news['outlink']['cid']) ?  $news['outlink']['tid'] : $news['outlink']['cid'];
					$replyCount = $this->Db->once_fetch_assoc("select count(*) as counts from pre_forum_post where first = 0 and invisible = 0 and tid=".$tid);
					$news['counts'] = intval($replyCount['counts']);
				}else{    
					$news['counts'] = 0;
				}
			...
			if($news['pic']&&$news['pic']!='nopic'){
				...
				if(strstr($news['pic'],'https')!==false){
                    $news['pic_style'] = 'normal';
                }
			}else{
				if(!strstr($news['pic'],'http')){
			...
			if($news['pic2'] && $news['pic3']){
				$news['topic_image'][] = $news['pic'];
				$news['topic_image'][] = 'http://www.yeeyi.com/'.$news['pic2'];
				$news['topic_image'][] = 'http://www.yeeyi.com/'.$news['pic3'];
				$news['pic_style'] = 'three';
			}
        ...
        /* add news advertise */
		$showAd = array(0,4,6,7,18,23,25,94);
		if ($startAid == 0 && in_array($cid,$showAd)) {
            ...
            if ($news_ad['status'] == 1){
				...
				if ($advertises['news_19']){
					$arr[] = $advertises['news_19'];
					add_show($advertises['news_19']['ad_id']);
					array_splice($tmpAry, $i+19, 0, $arr);
					$i++;
					unset($arr);
				}
            }
        }
		if ($page == 2 && in_array($cid,$showAd)) {
			$news_ad = get_ad('news',['channel'=>$cid]);
			if ($news_ad['status'] == 1){
				$advertises = $news_ad['data'];
				$i = 0;
				if ($advertises['news_25']){
					$arr[] = $advertises['news_25'];
					add_show($advertises['news_25']['ad_id']);
					array_splice($tmpAry, $i+5, 0, $arr);
					$i++;
					unset($arr);
				}
        ...
        if($cid==0 && $startAid==0){
 //           $tmpAry[3] = array('isCategory'=>1,'news_forms'=>$news_forms);
			$tmpAryCopy = array();
			foreach ($tmpAry as $k=>$v){
				if($k == 3) {
					$tmpAryCopy[] = array('isCategory'=>1,'news_forms'=>$news_forms);
				}
				$tmpAryCopy[] = $v;
			}
			$tmpAry = $tmpAryCopy; 
		...
		$return['newslist'] = $tmpAry;
		if ($startAid == 0) {
			$searchId = intval($this->Post['searchId']);
			if($searchId){
				$id = $searchId <5 ? $searchId+1 : 1;
			} else {
				$id = rand(1,5);
			}
			$sql="select id as searchId,searchkeyword from news_headnews where id=".$id;
			$searchkeyword= $this->Db->once_fetch_assoc($sql);
			$searchkeyword = changeCode($searchkeyword,'gbk','utf-8');
...
//		file_put_contents('huliang.txt',$authcode.'=='.MEMBER_ID);
		
		$aid = intval($this->Post['aid']);

        //Tony 20180314 先去缓存读取
        $newsAry = array();
        $newsContent_from_cache = getVar('newsContent_cache_'.$aid);

        if(!empty($newsContent_from_cache)){
            $newsAry = $newsContent_from_cache;
        }else{
            $newsAry = $this->Db->once_fetch_assoc("select news_article.aid,news_article.pic,news_article.lighten,news_article.cid,news_article.title,froms,editor,pic,description,pubdate,comments,good,content,bancom,type_1,type_2,type_3,type_4
            from news_article
            left join news_date
            on news_date.aid=news_article.aid
            where news_article.aid=".$aid);

            //Tony 20180314 这里不再关联 news_view 表
            //left join news_view
            //on news_view.aid = news_article.aid

            //把从数据库得到的结果存在缓存, 0.5小时
            if(!empty($newsAry)){
                setVar('newsContent_cache_'.$aid, $newsAry, 900);
            }
		...
		//更新阅读数  //Tony 20180314  加入 LOW_PRIORITY cancel
		...
		if(MEMBER_ID>0){
			//uid,id,idtype  //Tony 20180315 修改 * 为id
			$checkFavoriate = $this->Db->once_fetch_assoc("select id from pre_home_favorite where uid=".MEMBER_ID." and id=".$aid." and idtype='newsid'");
        ...
        //Tony 20180321  缓存topic_list， 0.5小时
        $topicInfoTmp = array();
        $newsContent_topiclist_cache = getVar('newsContent_topiclist_cache_'.$aid);
        if(!empty($newsContent_topiclist_cache)){
            $topicInfoTmp = $newsContent_topiclist_cache;
        }else{
            ...
            if(!empty($topicInfoTmp)){
                setVar('newsContent_topiclist_cache_'.$aid, $topicInfoTmp, 900);
            }
        }
        // -- end of 20180321  缓存缓存topic_list， 0.5小时
		...
		//Tony 20180321  缓存相关新闻， 24小时
        $newsContent_related_news_cache = getVar('newsContent_related_news_cache_'.$cid.'_'.$aid);
        $relevantNewsTmp = array();
        if(!empty($newsContent_related_news_cache)){
            $relevantNewsTmp = $newsContent_related_news_cache;
        }else{
            $relevantNewsTmp = $this->Db->fetch_all_assoc("select aid,title,pic,description,pubdate,froms,tourl,comments as counts from news_article where cid=".$cid." and aid!=".$aid." and delstate != 1 order by aid desc limit 0,10");
            if(!empty($relevantNewsTmp)){
                setVar('newsContent_related_news_cache_'.$cid.'_'.$aid, $relevantNewsTmp, 3600*24);
            }
        }
        // -- end of 20180321  缓存相关新闻， 24小时

		$relevantNews = array();
		foreach($relevantNewsTmp as $news){
			$news['isNews'] = 1;
			$news['counts'] = $news['counts'] ? $news['counts'] : 0;
        ...
        $return['views'] = '';  //Tony 20180315 浏览量强制返回为空
		$this->outjson($return);
	}
...
●	网友位置(IP解析后的地址文本，例如：来自山东青岛。如果解析不到地址则显示“来自火星”)(服务器端解决，澳洲这边地址要精确到suburb)
	...
	function getNewsReply(){
        ...
        //Tony 20190315 先判断有没有评论， 没有的话， 直接返回0
        $replyCount = $this->Db->once_fetch_assoc("SELECT COUNT(*) as counts from news_comment where aid=".$aid." and del_status=0");
        $reply_count_num = $replyCount['counts'];
        if($reply_count_num == 0){
            $return['status'] = 0;
		...
		$newsInfo = $this->Db->once_fetch_assoc("select bancom, title, pic from news_article where aid=".$aid);
		...
		if($rootid>0){
			...
			$amount = intval($this->Post['amount']);
			//root信息
			$rootAry = $this->Db->once_fetch_assoc("select a.id,a.username,a.userid,m.groupid,a.content,a.addtime,a.up as likes,a.del_status,a.address,a.commnum as replies,a.state from news_comment a left join pre_common_member m on a.userid = m.uid where a.id=".$rootid." and a.del_status=0 order by a.id desc");
...
//				$tmp['address'] = $addr? $addr: changeCode('火星网友','utf-8','gbk');
				$tmp['replies'] = $reply['replies'];
				//$tmp['upreply'] = NULL;
				$childReplyTmpAry = $this->Db->fetch_all_assoc("select a.id,a.username,a.userid,m.groupid,a.content,a.addtime,a.up as likes,a.del_status,a.address,a.commnum as replies,a.state from news_comment a left join pre_common_member m on m.uid = a.userid  where a.aid=".$aid." and a.rootid=".$reply['id']."  and a.del_status=0  order by a.up desc,a.id desc");
				$childReply = array();
...
//					$tmp2['address'] = $addr? $addr: changeCode('火星网友','utf-8','gbk');
			...
			if($startId>0){
				$replyTmp = $this->Db->fetch_all_assoc("select a.id,a.username,a.userid,m.groupid,a.content,a.addtime,a.up as likes,a.del_status,a.address,a.commnum as replies,a.state from news_comment a left join pre_common_member m on m.uid = a.userid where a.aid=".$aid." and a.rootid=0 and a.del_status=0 and a.id>$startId order by a.id desc limit 0,$amount");
			}
			else{
				$replyTmp = $this->Db->fetch_all_assoc("select a.id,a.username,a.userid,m.groupid,a.content,a.addtime,a.up as likes,a.del_status,a.address,a.commnum as replies,a.state from news_comment a left join pre_common_member m on m.uid = a.userid where a.aid=".$aid." and a.rootid=0 and a.del_status=0 order by a.id desc limit 0,$amount");
			}
			...
			foreach($replyTmp as $reply){
				...
				$tmp['likes'] = $reply['likes'];
				$tmp['addtime'] = $reply['addtime'];
				$tmp['addtimes'] = date("Y-m-d H:i:s",$reply['addtime']);
	...
	function doLink($url){
		...
		if(strstr($url,'yeeyi.com')){
			...
			if($aid == 0 && $tid == 0){
				...
				if(in_array($fid,array(325,89,651,161,632,631,630,714,304,643,642,641,142,627,626,625,624,623,622,680,305,677,681,291,653,635,634,633,660,715,716,656,657))){
					$array['tid'] = $tid;
