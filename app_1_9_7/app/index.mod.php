...
//		$redis->auth('e2XZ9abRF4');
//		$redis ->set( "test" , "Hello Worldlxi");
//		echo $redis ->get( "test");
//
//
//		die;
	...
	public function updateVersion()
	{
		...
		if ($current_version) {
			...
			if ($current_version_number< 195) { //有更新版本
	...
	function getCate()
	{
		...
		$newsType = array(
			...
			array('cid'=>'4','catname'=>'澳洲'),
			array('cid'=>'9999','catname'=>'视频','url'=>'http://interface.cloudepg.net/webapp_v2/sw_yyw/index.html'),
//			array('cid'=>'9999','catname'=>'视频','url'=>'http://www.baidu.com'),
			array('cid'=>'23','catname'=>'综合'),
			array('cid'=>'106','catname'=>'法律'),
			array('cid'=>'6','catname'=>'地产'),
			array('cid'=>'7','catname'=>'教育'),
...
//			array('cid'=>'25','catname'=>'公众号'),
	...
	function getSplash()        
	{
		$dev_type = intval($this->Post['dev_type']);
		$return = array();
        $rq = array(
            'model'=>'open',
        );
//        $result  =  postData('http://116.62.51.6:8005/api/get_ad',$rq);
        ...
        if ($open_ad['status'] == 1){   
			foreach($open_ad['data'] as $k=>$v){
				if ($dev_type == 1) {
					$open_ad['data'][$k]['pic'] = $open_ad['data'][$k]['picX'] ? $open_ad['data'][$k]['picX'] : $open_ad['data'][$k]['pic'];
				}
				unset($open_ad['data'][$k]['picX']);
			}
