...
class ModuleObject extends AppObject{
	...
	function Execute(){
		if($this->Act == 'uploadPic'){
			...
			$this->myFavorite();
		}elseif($this->Act == 'threadSearchAll'){
			$this->threadSearchAll();
		}elseif($this->Act == 'xcxQrcode'){
			$this->xcxQrcode();
		}
...
//个人收藏
	...
	function refreshThread(){
		...
		file_put_contents('fanhui.txt',var_export($output,true));
	...
	function getForumConfig(){
		...
		$cityInput = $this->Config['inPutCityXcx'];
//		$cityFilter = $this->Config['filterCity'];
		$forumList = $this->Config['forumListXcx'];
...
//		$forumAry['section_1'] = $this->getForumInfo($this->Config['life']);  //生活
		//print_r($forumAry['section_1']);die;
...
//		$return['hello'] = 'ceshi';
	...
	function getThreadModify(){
		...
		/*
            ...
            $picAry = array();
            $picResult = $this->Db->query("select aid,attachment from pre_forum_attachment_".$tableid." WHERE `tid`=".$tid);
            while($picInfo = $this->Db->fetch_assoc($picResult)){
                $threadTmp['pic'][] = 'http://www.yeeyi.com/bbs/data/attachment/forum/'.$picInfo['attachment'];
            }
        }
		*/
		//图片结束
...
//        echo json_encode($return);
		...
		$userThreadInfo = $this->Db->once_fetch_assoc("select tid from pre_forum_thread where tid=".$tid." and authorid=".$this->G['uid']);
		if($userThreadInfo){
	...
	private function getForumInfo($forumArray){
		...
		foreach($result as $result_value){
			if($result_value['fid_in']==92){
				$result_tmp = $result_value['types'];
			}

		}

		foreach($result_tmp as $k=>$type){
			if($type[1]=='问答'){
				unset($result_tmp[$k]);
				array_unshift( $result_tmp, $type);
			}
		}

		foreach($result as &$result_edit){
			if($result_edit['fid_in']==92){
				$result_edit['types'] = $result_tmp;
			}
		}
	 ...
	 * @version since v1.0
	 */
	function threadSearchAll(){

		$keyword = trim($this->Post['search']);
		$p = trim($this->Post['p']);
		$cityFilter = trim($this->Post['cityFilter']);
		$limit = trim($this->Post['limit']);

		$url = "https://www.yeeyi.com/forum/index.php?app=search&act=getsearch&keyword=$keyword&p=$p&limit=$limit&flag=FidAll&cityFilter=$cityFilter";
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $url);
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, FALSE);
		...
		$output = curl_exec($ch);
		curl_close($ch);
		$listAry = json_decode($output, true);
		$return = $return_ary =  array();
		foreach($listAry as $k=>$v){
			$return['fname']      = strip_tags($v['fname']);
			$return['subject']    = strip_tags($v['subject']);
			$return['author']     = strip_tags($v['author']);
			$return['dateline']   = $v['lastpost'];//需要变为这个字段
			$return['pic']         = $v['pic'];
			$return['tid']         = $v['tid'];
			$return_ary[] = $return;
		}
		...
		$returns['status'] = 0;
		$returns['threadlist'] = $return_ary;
		$this->outjson($returns);
	}
	/**
	 * @小程序之二维码
	 * @author kexun
	...
	function xcxQrcode() {
		...
		$tokenUrl="https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=wx63145c5fb29ff5f0&secret=e896c57aeb6b8cc9dbb3a791f00b190c";
		$getArr=array();
...
//		$url="https://api.weixin.qq.com/wxa/getwxacodeunlimit?access_token=".$access_token;
		...
		$filepath = "public/Qrcode.jpg";
		file_put_contents($filepath, $result);
		$returns['status'] = 0;
		$returns['pic'] = 'https://app.yeeyi.com.cn/public/Qrcode.jpg';
		$this->outjson($returns);
	}
