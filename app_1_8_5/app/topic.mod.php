	...
	function topicNewList(){
		...
		if ($page == 1){
            ...
            if ($topic_ad['status'] == 1){
				...
				$i = 0;
				if($this->Config['find_default']['find_3']){
					$arr[] = $this->Config['find_default']['find_3'];
					add_show($this->Config['find_default']['find_3']['ad_id']);
					array_splice($return['listAry'], $i+3, 0, $arr);
					$i++;
					unset($arr);
				}
				if($this->Config['find_default']['find_9']){
					$arr[] = $this->Config['find_default']['find_9'];
					add_show($this->Config['find_default']['find_9']['ad_id']);
					array_splice($return['listAry'], $i+9, 0, $arr);
					$i++;
					unset($arr);
				}
				if($this->Config['find_default']['find_13']){
					$arr[] = $this->Config['find_default']['find_13'];
					add_show($this->Config['find_default']['find_13']['ad_id']);
					array_splice($return['listAry'], $i+13, 0, $arr);
					$i++;
					unset($arr);
				}
...
//        echo json_encode($return);
	...
	function getThreadContent($tid,$topic_id,$cid){
		...
		$threadInfoTmp = $this->Db->once_fetch_assoc("select t.fid,t.tid,t.`subject`,t.author,t.authorid,t.closed,t.dateline,t.views,t.replies,t.attachment,p.message,p.anonymous,p.pid,m.groupid
                                                      ...
                                                      on t.authorid=m.uid
                                                      where t.tid=".$tid." and p.`first`=1 and p.`status`!=1 and t.`displayorder`>=0");
