...
//        }
        ...
        $this->where .= " and pre_forum_thread.`displayorder`>=0 ";
        $this->where .= " and pre_forum_field_service.city=".$this->Post['cityFilter']." ";
        if($start<1){
...
//            $this->where .= " and pre_forum_threadtop.froms=3 and pre_forum_threadtop.starttime<".time()." and pre_forum_threadtop.endtime>".time()." ";
//            $sqll = "select pre_forum_thread.tid from pre_forum_field_service left join pre_forum_thread  ".$use_index." on pre_forum_field_service.tid=pre_forum_thread.tid left join pre_forum_thread_pic on pre_forum_thread.tid = pre_forum_thread_pic.tid left join pre_forum_thread_refresh on pre_forum_thread.tid = pre_forum_thread_refresh.tid left join pre_forum_threadtop on pre_forum_thread.tid = pre_forum_threadtop.tid ".$this->where." order by lastpost desc limit 0,$amount";
//            $sqll = "select pre_forum_thread.tid from pre_forum_field_service left join pre_forum_thread  ".$use_index." on pre_forum_field_service.tid=pre_forum_thread.tid left join pre_forum_thread_pic on pre_forum_thread.tid = pre_forum_thread_pic.tid left join pre_forum_thread_refresh on pre_forum_thread.tid = pre_forum_thread_refresh.tid ".$where_raw." and pre_forum_thread_refresh.refresh>=$start";
//            $tidArray = $this->Db->fetch_all_assoc($sqll);
//            $tidString = "0";
//            foreach($tidArray as $v){
//                $tidString.=",".$v['tid'];
//            }
            $tidString = trim($this->Post['tidString']);
            if($tidString == "") {
                $sql = "select '651' as fid, '0' as iftopad, pre_forum_thread.tid,author,authorid,subject,pre_forum_thread_pic.description,dateline,pre_forum_thread_pic.pic,views,replies,'0' as likes,lastpost,pre_forum_field_service.typeid,pre_forum_field_service.company_area from pre_forum_field_service left join pre_forum_thread  ".$use_index." on pre_forum_field_service.tid=pre_forum_thread.tid left join pre_forum_thread_pic on pre_forum_thread.tid = pre_forum_thread_pic.tid left join pre_forum_thread_refresh on pre_forum_thread.tid = pre_forum_thread_refresh.tid ".$where_raw." and pre_forum_thread_refresh.refresh<$start order by lastpost desc limit 0,$amount";
            } else{
                $sql = "select '651' as fid, '0' as iftopad, pre_forum_thread.tid,author,authorid,subject,pre_forum_thread_pic.description,dateline,pre_forum_thread_pic.pic,views,replies,'0' as likes,lastpost,pre_forum_field_service.typeid,pre_forum_field_service.company_area from pre_forum_field_service left join pre_forum_thread  ".$use_index." on pre_forum_field_service.tid=pre_forum_thread.tid left join pre_forum_thread_pic on pre_forum_thread.tid = pre_forum_thread_pic.tid left join pre_forum_thread_refresh on pre_forum_thread.tid = pre_forum_thread_refresh.tid ".$where_raw." and pre_forum_thread_refresh.refresh<$start and pre_forum_thread.tid not in ($tidString) order by lastpost desc limit 0,$amount";
            }
    ...
    //多维数组去重
    function mn_array_unique($array) {
