...
//个人收藏
	...
	*/
	function refreshThread(){
		...
		$output['status'] = ($output['status'] == 2000) ? 2701 : 4000;
	...
	function getForumConfig(){
		...
		//板块列表
		// $forumHot = array();
		// foreach($this->Config['life'] as $forum){
		// 	// if($forum['fid'] == 277){
		// 	// 	continue;
		// 	// }
		...
		// $forumOther = array();
		// foreach($this->Config['work'] as $forum){
		// 	$threadsAry = $this->Db->once_fetch_assoc("select threads from pre_forum_forum where fid=".$forum['fid']);
		// 	$forum['views'] = intval($threadsAry['threads']);
		// 	$type = array();
		// 	$typeStrAry = $this->Db->once_fetch_assoc("SELECT threadtypes  FROM `pre_forum_forumfield` WHERE `fid` = ".$forum['fid']);
	...
	function test()
	{
		...
		$array = array();
		$array['offset'] = ($p-1)*100;
		$array['keyword'] = changeCode($keyword);
		$array['limit'] = 100;
	...
	function getThreadContent(){
		...
		$tid = intval($this->Post['tid']);
		$threadInfoTmp = $this->Db->once_fetch_assoc("select t.fid,t.tid,t.`subject`,t.author,t.authorid,t.closed,t.dateline,t.views,t.status,t.replies,t.attachment,p.message,p.anonymous,p.pid from `pre_forum_thread` t left join `pre_forum_post` p on t.tid=p.tid where t.tid=".$tid." and p.`first`=1 and t.`displayorder`>=0");
		...
		if($threadTmp){
			...
			*/
			...
			/* 2017.05.11 edit by allen qu 如果是招聘板块(161) 新增一个section_6 */
	...
	function getThreadReply(){
		...
		if ( in_array($info['status'], $status)) { //回复仅作者可见

			if ($info['authorid'] == MEMBER_ID) {
				...
				foreach($replyAry as $reply){
					/* 2017.05.08 edit by allen qu 仅有作者能看所有评论, 评论人可以看自己的评论, 评论的回复仅作者能看 */
					if(count($reply['upReply'])<1){
						unset($reply['upReply']);
					}else {

						//帖子的作者, 评论人, 回复人可以看到评论的回复
						if ($reply['authorid'] != MEMBER_ID || $info['authorid'] != MEMBER_ID || $reply['upReply']['authorid'] != MEMBER_ID) {

							$reply['upReply']['message'] = changeCode('评论内容仅作者可见','utf-8','gbk');
						}

					}
					if ($reply['authorid'] != MEMBER_ID) { //如果当前登录时评论人本人,可以看到自己的评论
	...
	function doReply(){
		...
		$this->Db->query("INSERT INTO pre_forum_post SET `fid`='".$threadInfo['fid']."' , `tid`='".$tid."' , `first`='0' , `author`='".changeCode(MEMBER_NAME, 'utf-8', 'gbk')."' , `authorid`='".$this->G['uid']."' , `subject`='' , `dateline`='".$time."' , `message`='".$message."' , `useip`='".$ip."' , `port`='63602' , `invisible`='0' , `anonymous`='0' , `usesig`='1' , `htmlon`='0' , `bbcodeoff`='".$bbcodeoff."' , `smileyoff`='-1' , `parseurloff`=0 , `attachment`='".$attNums."' , `status`='1024' , `pid`='".$pid."'");
