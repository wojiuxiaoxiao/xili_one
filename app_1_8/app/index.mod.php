...
class ModuleObject extends AppObject{
	...
	function getRate(){
		...
		$rateNew = @getPage("http://www.yeeyi.com/addon/ad/index_1/rate.txt");//接口地址待修改
		if($rateNew){
		...
		//天气
		$ip = client_ip();
		$city = trim($this->Post['city']) ? trim($this->Post['city']) : "墨尔本";  //搜索关键字
		$cityJson = @getPage("http://ip-api.com/json/".$ip);
		$cityAry = json_decode($cityJson,true);
		$city = $cityAry['city'];
...
**/
		...
		$return['froms'] = $this->Config['rate']['froms'];
		$return['pic'] = $this->Config['rate']['pic'];
		$return['url'] = $this->Config['rate']['url'];
